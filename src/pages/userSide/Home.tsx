...
export default function Home({}: Props) {
  ...
  return (
    <>
     ...
     <div className="p-5 h-screen w-screen flex flex-col justify-between overflow-hidden">
        {/* Desktop Icon Layout */}
        <div className="grid grid-cols-[repeat(auto-fill,_minmax(100px,_1fr))] gap-4 overflow-hidden">
          {currentItems.map((system, index) => (
            <div
              key={index}
              className="flex flex-col items-center justify-center p-3  rounded-md border border-transparent hover:border-gray-500 hover:bg-gray-100/30 backdrop-blur-lg transition-all duration-300 cursor-pointer"
              onClick={() => handleOpenApp(system.name)}
            >
              <div className="mb-2">{system.icon}</div>
              <span className="text-white text-xs">{system.name}</span>
